...
class Resize(object):
    ...
    def _resize_keypoints(self, results):
        ...
        for i, kp in enumerate(gt_keypoints):
            ...
            kp = kp.reshape((-1, 3))
            kp[:,0] *= results['scale_factor'][0]
            kp[:,1] *= results['scale_factor'][1]
            kp = kp.reshape((1, -1))
            gt_keypoints[i] = kp.tolist()
...
class RandomFlip(object):
    ...
    def __init__(self, flip_ratio=None, direction='horizontal',
                with_keypoints=False, gt_num_keypoints=17):
        ...
        self.with_keypoints = with_keypoints
        self.gt_num_keypoints = gt_num_keypoints
        self.count = 0
    ...
    def bbox_flip(self, bboxes, img_shape, direction):
        ...
        return flipped
# 翻转关键点
    def _flip_keypoints(self, keypoints, direction, img_shape):
        assert direction == 'horizontal'
        keypoints_np = np.array(keypoints).reshape((len(keypoints),-1,3))
        w = img_shape[1]
        ...
        keypoints_np = keypoints_np.tolist()
        return keypoints_np
        ...
        if results['flip']:
            ...
            if self.with_keypoints:
                results['gt_keypoints'] = self._flip_keypoints(
                    results['gt_keypoints'],  direction=results['flip_direction'], img_shape=results['img_shape'])

            # visualize
            # img_patch = results['img']
